...
func Generate(pkg string, model *FSMSchema) ([]byte, error) {
	...
	//   - Each state should have a method in the builder that takes a function that takes a context, an object for building transitions to the next state, and the input(s) for the current state. The function should return a transition to the next state, or an error.
	//   - The method should be named {StateName}State
	//   - The builder stage should be named {SchemaName}FSMBuilder_{StateName}Stage
	//   - The final stage should have a Build method that returns the FSM.
