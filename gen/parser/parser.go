...
// Parser is a table-driven LALR parser for fsm.
type Parser struct {
	eh       ErrorHandler
	listener Listener
...
func (p *Parser) parse(start, end int8, lexer *Lexer) error {
	...
	for state != end {
		...
		if action == -2 || state == -1 {
			...
			p.recovering = 4
			stack = p.recoverFromError(lexer, stack, end)
			if stack == nil {
				return lastErr
			}
...
func reduceAll(stack []stackEntry, state int8, symbol int32, endState int8) (int8, bool) {
	if symbol == noToken {
...
func (p *Parser) recoverFromError(lexer *Lexer, stack []stackEntry, endState int8) []stackEntry {
